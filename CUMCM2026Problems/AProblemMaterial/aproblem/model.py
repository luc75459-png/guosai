...
class DryingModel:
    ...
    def stable_time_step(
        ...
        conductivity_faces = harmonic_mean(
        ...
        if self.include_end_faces:
            heat_outflow += (
                2.0 * self.heat_transfer_coefficient / self.cylinder_length_m
            ) * geometry.volumes_per_length_m2
        temperature_rates = heat_outflow / heat_capacity_per_length

        diffusivity_faces = harmonic_mean(
        ...
        if self.include_end_faces:
            moisture_outflow += (
                2.0 * self.mass_transfer_coefficient / self.cylinder_length_m
            ) * geometry.volumes_per_length_m2
